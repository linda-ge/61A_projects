...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        new_expr = scheme_eval(first,env)
        check_procedure(new_expr)
        return Procedure.eval_call(new_expr,rest,env)
        # END PROBLEM 5
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        if operands is nil:
            return scheme_apply(self,nil,env)
        mapped_rest = Pair.map(operands, lambda x: scheme_eval(x, env))
        return scheme_apply(self,mapped_rest,env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        Frame.define(env, target, target_value)
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        new_expr = Pair(target.second, expressions.second)
        new_val = do_lambda_form(new_expr,env)
        Frame.define(env, target.first, new_val)
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions.second is nil and scheme_truep(eval_var):
        return eval_var
    elif scheme_truep(eval_var):
...
def do_let_form(expressions, env):
    ...
    #make a frame with the bindings
    return eval_all(expressions.second, let_env)
    #eval all of the following expressions in the made frame.
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    variables, values = nil, nil
    while bindings is not nil:
        check_form(bindings.first,2,2)
        value = scheme_eval(bindings.first.second.first,env)
        variable = bindings.first.first
        variables = Pair(variable, variables)
