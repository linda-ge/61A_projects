...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        #If the Place has an exit, then the exit's entrance is set to that Place. Take it literally. You were running into errors when the Place didn't have an exit (exit was None), so the program timed out.
        # END Problem 2
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                if self.ant.can_contain(insect): #add bodyguard first
                    insect.place = self
                    return self.ant.contain_ant(insect)
                elif insect.can_contain(self.ant): #add bodyguard second
                    insect.place = self
                    blep = insect.contain_ant(self.ant)
                    self.ant = insect
                    return blep
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect: #if the current ant is the insect u want to remove
                if hasattr(self.ant, 'container') and self.ant.container: #if the current ant has a container attribute and is containing
                    self.ant = self.ant.ant #the ant in place is the contained ant
                elif isinstance(insect, QueenAnt) and self.ant.real: #otherwise, if the insect is an instance of a queenant and is real
                        return #cut the function short, return nothing
                else:
                    self.ant = None
            else:
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if isinstance(insect, QueenAnt) and self.ant.ant.real:
                        return
                    else:
                        self.ant.ant = None
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        current = self.place
        curr_range = 0
        while current is not None and current is not hive:
            if len(current.bees)!=0:
                if isinstance(self, LongThrower):
                    ...
                    return random_or_none(current.bees)
            current, curr_range = current.entrance, curr_range+1
        return None
        # END Problem 5
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.watersafe:
            ...
            #originally didn't work out b/c you had called self, insect.armor and especially b/c you had capitalized insect.
        ...
        if self.real is False:
            ...
            while current is not None:
                if current.ant is not None:
                    if current.ant not in self.antz:
                        current.ant.damage *= 2
                        self.antz.append(current.ant)
                    if hasattr(current.ant, 'container') and current.ant.container and current.ant.ant is not None and current.ant.ant not in self.antz: #ant in the container ant
                        current.ant.ant.damage *= 2
                        self.antz.append(current.ant.ant)
                current = current.exit
#should i decrement everything at the end of the turn?
        # END Problem 13
